 ...
 *
 * genehisto relies on data created by skfit.
 *
 * creates big genehisto.csv file with one line per chromosome
 * plus _aa.csv (disabled right now)
 * plus _codons.csv (with statistics per codon in the reading frame)
 * plus a .dot file with a state transition diagram for codons
 */
...
int main(int argc, char**argv)
{
  ...
  auto doFunc=[&argc, &argv, &n, &results, &codgccsv]() {
     ...
     for(int j = n++; j < argc; j = n++) {
       try {
         ReferenceGenome rg(argv[j]);
         string garname = argv[j];
         boost::replace_all(garname, ".fna", ".gff");
  
         cout<<"Done reading genome from "<<argv[j]<<", have "<<rg.numChromosomes()<<" chromosomes, "<<
         ...
         for(const auto& c : rg.getAllChromosomes()) {
           ...
           */
           ...
           for(const auto& a : annotations) {
             if(a.type=="gene" && a.gene_biotype=="protein_coding") {
           ...
           tmpstr<<c.first<<";"<<1.0*gCodonCount/(gCodonCount+cCodonCount) << ";"<< 1.0*cCodonCount/(gCodonCount+cCodonCount)<<";"<<1.0*tCodonCount/(tCodonCount+aCodonCount) << ";"<< 1.0*aCodonCount/(tCodonCount+aCodonCount)<<";";
           tmpstr<<1.0*gCodonCount/totCodonCount<<";";
           tmpstr<<1.0*cCodonCount/totCodonCount<<";";
           tmpstr<<1.0*tCodonCount/totCodonCount<<";";
           tmpstr<<1.0*aCodonCount/totCodonCount;
           ...
           for(int p =0 ; p < 4; ++p)
             tmpstr<<";"<<1.0*leadnucs[p]/totleadnucs;
           for(int p =0 ; p < 4; ++p)
             tmpstr<<";"<<1.0*lagnucs[p]/totlagnucs;
           ...
           codoncsv<<"codon,aa,sense,antisense,ssense,santisense,total,antitotal,dup,gcscore,tascore"<<endl;
           set<NucleotideStore> done;
           for(const auto& codo : codons) {
             codoncsv<<codo.first<<","<<DNAToAminoAcid(codo.first.toASCII().c_str())<<","<<codo.second<<","<<anticodons[codo.first];
